 ...
 */
public class DataCmdHandler extends AbstractLogEnabled implements
    CommandHandler {
    ...
    private MailHeaders processMailHeaders(SMTPSession session,
        MailHeaders headers) throws MessagingException {
    ...
    if (!headers.isSet(RFC2822Headers.FROM)
        ...
        headers.setHeader(RFC2822Headers.FROM, session.getState().get(
    ...
    headerLineBuffer.append("          by ").append(
        ...
        .append(SOFTWARE_TYPE).append(") with SMTP ID ").append(
    ...
    private void processMail(SMTPSession session, MailHeaders headers,
        InputStream msgIn) throws MessagingException {
    ByteArrayInputStream headersIn = null;
    MailImpl mail = null;
    List recipientCollection = null;
    try {
        headersIn = new ByteArrayInputStream(headers.toByteArray());
        recipientCollection = (List) session.getState().get(
            SMTPSession.RCPT_LIST);
        mail = new MailImpl(session.getConfigurationData().getMailServer()
            .getId(), (MailAddress) session.getState().get(
            SMTPSession.SENDER), recipientCollection,
            new SequenceInputStream(new SequenceInputStream(headersIn,
                msgIn), new ReaderInputStream(new StringReader(
                "\r\n"))));
        ...
        mail.setAttribute(SMTP_AUTH_USER_ATTRIBUTE_NAME, session
            .getUser());
        }
