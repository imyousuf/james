...
/**
 * Handler called upon receipt of a ADDHAM command
 *
 */
public class AddHamCmdHandler implements CommandHandler{
    private final static String COMMAND_NAME = "ADDHAM";
    private CommandHelp help = new CommandHelp("addham dir/mbox [directory/mbox]","feed the BayesianAnalysisFeeder with the content of the directory or mbox file as HAM");
    ...
    public RemoteManagerResponse onCommand(RemoteManagerSession session, String command, String parameters) {
        ...
        try {
            ...
            if (args[0].equalsIgnoreCase("DIR")) {
                ...
                count = bayesianAnalyzerManagement.addHamFromMbox(args[1]);
            } else {
                response = new RemoteManagerResponse("Usage: " + getHelp().getSyntax());
                return response;
            }
            response = new RemoteManagerResponse("Feed the BayesianAnalysis with " + count + " HAM");
